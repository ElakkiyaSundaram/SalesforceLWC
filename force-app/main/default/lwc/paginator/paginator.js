...
export default class MyLightningWebComponent extends LightningElement {
    ...
    get disablePreviousPageButton() {
        return this.pageNumber <= 1;
    }

    get disableNextPageButton() {
        return this.pageNumber >= this.totalPages;
    }

    get previousButtonsStyle() {
        return `slds-m-left_x-small custom-button ${this.disablePreviousPageButton ? 'custom-button-disabled' : ''}`;
    }

    get nextButtonsStyle() {
        return `slds-m-left_x-small custom-button ${this.disableNextPageButton ? 'custom-button-disabled' : ''}`;
    }
    ...
    reloadData(newPageNumber) {
        let operationType = this.getOperationType(newPageNumber);
